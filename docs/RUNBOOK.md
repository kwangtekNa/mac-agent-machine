...
- **곁방(에이전트 간 대화)**(2026-09-14, `docs/PROTOCOL.md` 6.6): 에이전트가 답변에서 다른 에이전트를 `@멘션` 하면 그 대화는 자동으로 **곁방**으로 갈라진다(`kind: "side"`, 이름은 `민수 ↔ 지연`). 그룹방에는 원본 답변과 연결 카드만 남는다 — 곁방이 열릴 때 한 줄, 대화가 끝나면 "대화 N건 · 결론: …" 한 줄. 사람도 곁방에 들어가 직접 끼어들 수 있고(방 목록의 "에이전트 간" 섹션 또는 연결 카드 탭), 멘션 없이 쓰면 참가자 전원이 답한다. 같은 조합이면 늘 같은 방을 재사용하고 방은 지우지 않는다. 참가자가 `settings.sideRoomMaxParticipants`(기본 3, `PATCH /api/v1/teams/:id` 로 2~8)를 넘는 호출은 공지·브로드캐스트로 보고 곁방을 만들지 않고 그룹방에 그대로 둔다. 변경 카드는 곁방에서 한 작업이라도 그룹방에 올라오므로 머지는 한곳에서 본다. 곁방에 참가하지 않은 팀원의 턴 입력에는 그 대화가 들어가지 않는다(맥락 절약, ADR-018).
- **방 읽기(작업 카드 접기)**(2026-09-15, `docs/IOS.md` 10.12): 방을 열면 최신 메시지가 먼저 보이고, 연속된 승인·변경·공지 카드는 한 줄(`변경 3건` + 노란 `머지 대기 N건`)로 접혀 있다 — 탭하면 그 자리에서 개별 카드가 펼쳐지고(머지·거절 버튼도 그대로) 다시 탭하면 접힌다. **대기 중 승인은 접히지 않고 항상 펼쳐져 있다**(사람이 눌러야 그 팀원이 계속 진행한다). 접는 건 화면뿐이라 방 로그에는 모든 카드가 그대로 남는다.
...
- **재시작**: agent-host 가 다시 뜨면 진행 중이던 턴은 취소되고 그룹방에 안내가 올라온다. `merging` 이던 카드는 git 을 대조해 `merged` 또는 `ready` 로, 브랜치가 바뀐 `ready` 카드는 `stale` 로 정리된다. (2026-09-15, ADR-019) 답을 받지 못한 승인 카드는 세션의 실제 대기 목록과 대조해 **"시스템이 취소함"**(`by: "system"`) 으로 정리된다 — 팀원 기억 초기화·제거 때도 같다. 에이전트는 그 턴을 잃으므로 필요하면 방에 다시 요청하면 된다(살아 있는 승인은 시한 없이 그대로 기다린다).
- **개발 확인**: `bash scripts/dev-smoke.sh` 의 25단계가 곁방(분리 → 연결 카드 → 재사용 → 참가자 전원 디스패치 → 맥락 격리)을, 15~21단계가 Fake 어댑터로 팀 생성 → 방 WS → 멘션 디스패치 → 변경 카드 → DM → 팀원 제어(full-auto 는 승인 카드 없음) → 머지 → 삭제를 통과한다. 실제 CLI 는 `MAM_IT_CLAUDE=1`/`MAM_IT_CODEX=1 npx vitest run --root packages/server test/teams/integration.test.ts`(비용 수 센트).

...
- **턴 입력**: 팀원 세션에 보내는 턴 텍스트는 (1) 방 맥락, (2) 이번 메시지 순이다. (2026-09-14 추가) 한 팀원의 **맥락 방**은 그룹방 + 자기 DM 방 + **자기가 참가한 곁방 전부**이며, 이미 본 메시지 표시(`lastSeen`)는 방마다 따로 둔다. 맥락은 그 방의 최근 메시지를 `contextMaxMessages`(기본 40)개, 합쳐서 12,000자 이내로 잘라(오래된 것부터 버림) 한 줄씩 접두어를 붙인다: 사용자 `[#전체] 사용자: …` / `[DM] 사용자: …`, 에이전트 `[#전체] @민수(개발자): …`(`@handle(roleLabel)`), 시스템 `[#전체] 시스템: …`. 팀원이 이미 본 메시지(자기 세션에 전달된 것)는 다시 넣지 않는다. (2026-09-14 추가, 2026-09-15 갱신) 턴 입력에는 승인·변경 카드가 **전혀** 들어가지 않는다(자기 것 포함). 방 화면에는 남는다 — 사람이 승인·머지해야 한다. 자기가 실행한 명령·바꾼 파일은 그 팀원 세션 타임라인에 이미 있고, 남의 카드는 제목이 bash 명령 원문이라 길기만 하다. 시스템 메시지는 홉 상한·재시작·곁방 연결·머지 충돌 지시를 나르므로 그대로 넣는다. 예외는 트리거 하나다 — 디스패치를 일으킨 메시지가 승인·변경 카드면 필터와 무관하게 마지막 줄(`[#전체] 시스템: 지연의 승인 요청 '…' — 대기 중`)로 들어간다.
...
- **승인 미러링**: 팀원 세션의 `approval.requested` 는 디스패치가 시작된 방에 `kind: "approval"` 메시지로 미러링하고 `approval.resolved` 때 `room.message.updated`. 응답은 6.3 대로 기존 세션 API. (2026-09-15 추가) 서버가 다시 열릴 때와 팀원 세션이 닫힐 때(초기화·제거) 서버는 방의 미해결 승인 카드를 그 세션의 실제 대기 목록과 맞춰 보고, 세션에 없는 카드는 `{ optionId: "abort", by: "system" }` 으로 정리해 `room.message.updated` 로 내보낸다(카드는 지우지 않고 `resolution` 만 채운다). 승인 자체에는 시한이 없어 살아 있는 요청은 사람이 답할 때까지 `resolution: null` 로 남는다.
